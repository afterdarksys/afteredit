...
export default function SearchPanel({root,onOpen}:{root:string;onOpen:(path:string,line:number)=>void}){
 ...
 async function search(){setBusy(true);try{const r=await invoke<{hits:typeof hits;truncated:boolean}>('workspace_search',{root,query});setHits(r.hits);setStatus(`${r.hits.length} matches${r.truncated?' (search limit reached)':''}`);}catch(e){setStatus(String(e));}finally{setBusy(false);}}
