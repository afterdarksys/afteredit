...
export function useDebugger(root:string,file:string,onNavigate:(path:string,line:number)=>void){
 ...
 const clearInspection=()=>{epoch.current++;tools.invalid();setStopReason('');setFrames([]);setFrame(undefined);setScopes([]);setVariables({});setEvaluation('');};
 const tools=useDebugTools({root,phase,frame,thread,frames,variables,caps},(command,args)=>request(command,args));
 const request=async(command:string,args:Record<string,any>={})=>{if(session.current===null)throw new Error('No active debugger');return tools.journal.request(command,()=>invoke<any>('dap_request',{session:session.current,command,arguments:args}));};
 ...
 async function stop(){generation.current++;abortStartup.current(new Error('Debug start cancelled'));abortStartup.current=()=>{};const id=session.current;session.current=null;unlisten.current();unlisten.current=()=>{};clearInspection();setThreads([]);setThread(undefined);setPoints(v=>v.map(({verified:_v,message:_m,adapterId:_id,...p})=>p));setPhase('idle');setBusy(false);phaseRef.current='idle';if(id!==null){try{await Promise.race([tools.journal.request('disconnect',()=>invoke('dap_request',{session:id,command:'disconnect',arguments:{terminateDebuggee:mode.current==='launch'}})),new Promise(resolve=>setTimeout(resolve,1500))]);}catch{}await invoke('dap_stop',{session:id}).catch(report);}}
 useEffect(()=>{return()=>{void stop();};},[root]);
 async function start(text:string,filters:string[]){
  if(session.current!==null&&phaseRef.current==='ended')await stop();if(session.current!==null||busy)return;const run=++generation.current;tools.reset();setBusy(true);setError('');setOutput('');setCaps({});capsRef.current={};clearInspection();setPhase('starting');phaseRef.current='starting';
  ...
  try{
   ...
   const off=await listen<{session:number;message:any}>('dap:event',({payload})=>{
    if(run!==generation.current||payload.session!==session.current)return;const m=payload.message,b=m.body??{};tools.journal.event(m.event,String(b.reason??b.description??''));
   ...
   const req=async(c:string,a:Record<string,any>)=>{if(run!==generation.current)throw new Error('Debug start cancelled');return tools.journal.request(c,()=>invoke<any>('dap_request',{session:id,command:c,arguments:a}));};
 ...
 async function changePoints(next:Breakpoint[]){
  ...
  if(session.current!==null&&['running','paused'].includes(phaseRef.current)){
   try{for(const path of new Set([...previous,...next].filter(p=>p.path.startsWith(root+'/')).map(p=>p.path))){if(run!==generation.current||session.current!==connection)return;const reply=await tools.journal.request('setBreakpoints',()=>invoke<any>('dap_request',{session:connection,command:'setBreakpoints',arguments:breakpointArguments(path,next,capsRef.current)}));if(run!==generation.current)return;updateVerified(path,reply.breakpoints??[]);}}catch(e){report(e);}
  }
 }
 ...
 return {tools,stopReason,phase,error,output,caps,points,threads,thread,frames,frame,scopes,variables,evaluation,busy,start,stop,changePoints,toggle,selectThread,selectFrame,loadVariables,control,evaluate,setVariable,report,root};
}
