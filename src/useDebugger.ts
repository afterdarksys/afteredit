...
export function useDebugger(root:string,file:string,onNavigate:(path:string,line:number)=>void){
 ...
 const session=useRef<number|null>(null),generation=useRef(0),epoch=useRef(0),initialized=useRef<()=>void>(()=>{}),capsRef=useRef<Record<string,any>>({}),mode=useRef('launch'),unlisten=useRef<()=>void>(()=>{}),phaseRef=useRef(phase),pointsRef=useRef(points),navigation=useRef(onNavigate);
 ...
 async function stop(){generation.current++;const id=session.current;session.current=null;unlisten.current();unlisten.current=()=>{};clearInspection();setThreads([]);setThread(undefined);setPoints(v=>v.map(({verified:_v,message:_m,adapterId:_id,...p})=>p));setPhase('idle');setBusy(false);phaseRef.current='idle';if(id!==null){try{await Promise.race([invoke('dap_request',{session:id,command:'disconnect',arguments:{terminateDebuggee:mode.current==='launch'}}),new Promise(resolve=>setTimeout(resolve,1500))]);}catch{}await invoke('dap_stop',{session:id}).catch(report);}}
 ...
 async function start(text:string,filters:string[]){
  ...
  try{
   ...
   const ready=new Promise<void>((resolve,reject)=>{initialized.current=resolve;timer=setTimeout(()=>reject(new Error('Adapter did not send initialized within 30 seconds')),30000);});
   ...
   const off=await listen<{session:number;message:any}>('dap:event',({payload})=>{
    ...
    if(m.event==='terminated'||m.event==='aftereditClosed'){clearInspection();setPhase('ended');phaseRef.current='ended';setBusy(false);if(m.event==='aftereditClosed')setError(String(b.reason??'Adapter closed'));}
   });
  ...
  }catch(e){if(run===generation.current){await stop();report(e);}}finally{if(timer)clearTimeout(timer);if(run===generation.current)setBusy(false);}
 }
