...
import { Code, Terminal, GitBranch, LayoutPanelLeft, Search, Bug, Files, ChevronRight, Zap, Settings } from "lucide-react";
...
function App() {
  ...
  const [layout, setLayout] = useState<"stacked" | "side-by-side">("side-by-side");
  const [osTheme, setOsTheme] = useState<"mac" | "win">("mac");
  const [pairProgrammingOn, setPairProgrammingOn] = useState(true);
  
  // Settings & Command Palette State
  const [showCommandPalette, setShowCommandPalette] = useState(false);

  const terminalRef = useRef<HTMLDivElement>(null);
  const [code, setCode] = useState(`export function App() { return <div>Hello Settings</div>; }`);

  // Handle Cmd+Shift+P to open command palette
  useEffect(() => {
  ...
  useEffect(() => {
    document.body.className = `theme-\${osTheme}`;
    if (terminalRef.current) {
      terminalRef.current.innerHTML = '';
      const term = new XTerm({
        theme: { background: osTheme === 'mac' ? 'rgba(30,30,30,0.8)' : '#1e1e1e' },
      ...
      fitAddon.fit();
      term.write('AfterEdit Native PTY\r\n\x1b[32mryan@mbp\x1b[0m:\x1b[34m~/afteredit\x1b[0m$ ');
    }
  }, [layout, osTheme]);
  ...
  const renderEditorContent = () => {
        ...
        <div className="preferences-ui">
          ...
          <div className="pref-section">
            ...
            <div className="pref-row">
              ...
              <select className="pref-select" value={osTheme} onChange={(e) => setOsTheme(e.target.value as "mac"|"win")}>
            ...
            <div className="pref-row">
              ...
              <select className="pref-select" value={layout} onChange={(e) => setLayout(e.target.value as any)}>
          ...
          <div className="pref-section">
            ...
            <div className="pref-row">
              ...
              <select className="pref-select">
                <option>Claude 3.5 Haiku (Fastest)</option>
    ...
    return (
      <Editor height="100%" theme="vs-dark" defaultLanguage="typescript" value={code} onChange={(v) => setCode(v || "")} />
    );
  };

  return (
    <div className={`app-container \${osTheme}`}>
