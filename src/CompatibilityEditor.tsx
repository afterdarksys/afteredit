...
export default function CompatibilityEditor(props:Props){
 ...
 const document=()=>{const p=latest.current;return {path:p.path,value:p.value,language:languageForFilename(p.path),options:editorOptions(p.options),revision:revision.current};};
 useEffect(()=>{
 ...
 },[props.path,props.value,props.options,ready]);
