...
export default function CompatibilityEditor(props:Props){
 ...
 const document=()=>{const p=latest.current;return {root:p.root,settingsJSON:p.settingsJSON,path:p.path,value:p.value,language:languageForFilename(p.path),options:{...editorOptions(p.options),...accessibleEditorOptions(accessRef.current,p.path)},theme:accessibleEditorTheme(accessRef.current,p.options.theme),revision:revision.current};};
 useEffect(()=>{
  ...
  const receive=(e:MessageEvent)=>{
   ...
   const data=e.data;
   if(data.type==='settings'&&typeof data.settingsJSON==='string'){try{latest.current.onSettingsChange(data.settingsJSON);}catch(e){setStatus(String(e));}}
