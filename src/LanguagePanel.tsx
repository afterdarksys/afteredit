...
export default function LanguagePanel({root,configured,connected,onChange}:{root:string;configured:Record<string,ServerConfig>;connected:ConnectedServer[];onChange:(servers:ConnectedServer[])=>void}){
 ...
 async function connect(){setBusy(true);let id:number|undefined;try{
  ...
  onChange([...connected,{...started,root,language,capabilities:result.capabilities}]);setStatus(`Connected ${language}`);
 ...
 <p>Store server definitions under languageServers in .afteredit.json for project/directory overrides. Up to eight servers may be connected. Dynamic capability registration, code actions, semantic tokens and debugging are not yet supported.</p></section>;
}
